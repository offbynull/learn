...
with open('/home/user/Downloads/dataset_240282_8(1).txt', mode='r', encoding='utf-8') as f:
    data = f.read()
...
while len(leaderboard) > 0:
    ...
    for p in leaderboard:
        if sum(p) > max(spectrum.elements()):
            ...
            if linear_score(Counter(p), spectrum) > linear_score(Counter(leader_peptide), spectrum):
                leader_peptide = p
